...
export class PoidLourdsListComponent implements OnInit {
  ...
  public tousLesProduits: PoidLourdsProduit[] = [];

  // Produits affichés après filtres
  produitsFiltres: PoidLourdsProduit[] = [];
  ...
  toggleFavori(produit: PoidLourdsProduit, event: Event): void {
  ...
  // Panier
  // -------------------------------------------------------
  ajouterAuPanier(produit: PoidLourdsProduit, event: Event): void {
    event.stopPropagation();
  ...
  isStockFaible(p: PoidLourdsProduit): boolean { return p.stock > 0 && p.stock <= 5; }
